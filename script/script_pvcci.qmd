...
# adm0
adm0_gadm <- read_sf('C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/gadm_410-levels/gadm_410-levels.gpkg', layer ='ADM_0') |> 
  clean_names() |> 
...
# adm1
adm1_gadm <- read_sf('C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/gadm_410-levels/gadm_410-levels.gpkg', layer ='ADM_1') |> 
  clean_names() |> 
  mutate(gid_0 = ifelse(gid_0 == 'XKO', 'XKX', gid_0)) |> 
  # filter(!gid_0 == 'ALA') |> # remove åland, as part of finland
  rename(iso3 = gid_0)
...
#adm2
adm2_gadm <- read_sf('C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/gadm_410-levels/gadm_410-levels.gpkg', layer ='ADM_2') |> 
  clean_names() |> 
  mutate(gid_0 = ifelse(gid_0 == 'XKO', 'XKX', gid_0)) |> 
  # filter(!gid_0 == 'ALA') |> # remove åland, as part of finland
  rename(iso3 = gid_0)
...
adm2_gadm_v3.6 <- st_read("C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/gadm_adm2p/gadm_adm2p.shp") |>
  clean_names() |> 
...
## Merge GADM 3.6 and 4.1

```{r}

g36 <- adm2_gadm_v3.6
g410 <- adm2_gadm_clean
...
# saveRDS(g36, g410,
#      "C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/gadm_36_410.RDS")
...
g36 <- mutate(g36, g36_id = row_number())
g410 <- mutate(g410, g410_id = row_number())

# identify duplicates in each set
dup36 <- g36 |> st_set_geometry(NULL) |> count(hasc) |> filter(!is.na(hasc) & n>1)
dup4  <- g410 |> st_set_geometry(NULL) |> count(hasc) |> filter(!is.na(hasc) & n>1)
...
exact_name_matches <- inner_join(
  g36 |> 
    st_set_geometry(NULL) |> 
    filter(!hasc %in% g36_hasc_unique$hasc),
  g410 |> 
    st_set_geometry(NULL) |> 
    filter(!hasc %in% g410_hasc_unique$hasc),
...
exact_matches <- bind_rows(
  ...
  arrange(match_type)  # exact_hasc first

# rows in g36 not yet matched
unmatched_g36 <- g36 |> 
  filter(!g36_id %in% exact_matches$g36_id)

# rows in g410 not yet matched
unmatched_g410 <- g410 |> 
  filter(!g410_id %in% exact_matches$g410_id)
...
fuzzy_matches_list <- lapply(thresholds, function(thresh){
  stringdist_inner_join(
    unmatched_g36 |> select(g36_id, iso3, adm1_n, adm2_n),
    unmatched_g410 |> select(g410_id, iso3, adm1_n, adm2_n),
...
save(fuzzy_matches_list, fuzzy_matches,
     file = "C:/Users/pauvernu/Seafile/library/chap_two_data/gadm/fuzzy_matches.RData")
...
# fuzzy matches w/out duplicate, i.e. many_to_many==FALSE
exact_fuzzy <- filter(fuzzy_matches, many_to_many==FALSE)

# fuzzy matches w/ duplicates, i.e. many_to_many==TRUE
fuzzy_unmatched <- filter(fuzzy_matches, 
       !g36_id %in% exact_fuzzy$g36_id | !g410_id %in% exact_fuzzy$g410_id)

fuzzy_unmatched <- fuzzy_unmatched |> 
  group_by(g36_id) |> 
  mutate(has_exact_adm2 = any(adm2_n.dist==0)) |> 
  ungroup()

# fuzzy matches w/ duplicates, but exact matching at adm2 level
fuzzy_many_exact_adm2 <- filter(fuzzy_unmatched, has_exact_adm2) |> 
  mutate(match_type = paste0(match_type, "_adm2"))

fuzzy_unmatched <- filter(fuzzy_unmatched,
       !g36_id %in% fuzzy_many_exact_adm2$g36_id | !g410_id %in% fuzzy_many_exact_adm2$g410_id)
...
fuzzy_unmatched |> 
  group_by(g36_id, match_type) |> 
  mutate(split = ifelse(
    length(g36_id, match_type)>1 & prod(n_matches_split) == 1, TRUE, FALSE)) |> 
  View()
...
fuzzy_unmatched <- fuzzy_unmatched |> 
  mutate(split = ifelse(prod(n_matches_split) == 1, TRUE, FALSE), 
         .by = g410_id) |> 
  mutate(merge = ifelse(prod(n_matches_merge) == 1, TRUE, FALSE), 
         .by = g36_id)

fuzzy_many_split <- filter(fuzzy_unmatched, split) |> 
  mutate(match_type = paste0(match_type, "_split"))  
fuzzy_many_merge <- filter(fuzzy_unmatched, merge) |> 
  mutate(match_type = paste0(match_type, "_merge"))

filter(fuzzy_unmatched, !split & !merge) |> 
  View()



# group_by(gXX_id)
# arrange(match_type, dist)
# slice_min(match_type, dist) # check if keep multiple or single observation (we want to keep multiple observations)

# stack all matches
all_matches_combined <- bind_rows(
  exact_matches,
  exact_fuzzy
) |> 
